...
goalie_season_beta <- goalie_shot_level_prep %>%
        ...
        summarise(xG_Lift_100Shots = (sum(xG) -sum(as.numeric(goal)-1)) / (n() / 100),
                  ...
                  Shots = n(),
                  `Observed Save % (Non-Rebound Shots)` = (NonRebound_Shots - sum(as.numeric(goal)-1)) / NonRebound_Shots, 
                  `Observed Save % (All Shots)` = (Shots - sum(as.numeric(goal)-1)) / Shots, 
...
goalie_cumulative <- goalie_shot_level_prep %>%
        ...
        arrange(SA_Goalie, season2, as.character(gcode),seconds) %>%
        group_by(SA_Goalie) %>%
        mutate(mean_xG = cumsum(xG) / seq(n()),
              ...
              Cum_Shots = seq(n()),
              Cum_GA = cumsum(as.numeric(goal)-1),
          
              cum_xG_FirstShot = cumsum(xG_FirstShot),
              cum_xG = cumsum(xG),
              cum_xG_RB = cumsum(pred.rebound * rebound_goal_probability),
              cum_xG_raw = cumsum(xG.raw),
...
cumulative_lift_plot2 <- goalie_cumulative %>%
      group_by(SA_Goalie, season2, gcode) %>%
      top_n(1, Cum_Shots) %>%
      ungroup() %>%
      select(Cum_Shots, season, ends_with("_lift")) %>%
      ...
      theme_standard() + ggthemes::scale_color_gdocs() + ggthemes::scale_fill_gdocs() +
      labs(title = "Matt Murray Posterior Lift Over Expected by Game",
         y = "Posterior Lift Over Expected", x="Cumulative Shots Against", fill="Prior Shots", color="Season")

ggsave(filename="/Users/colander1/Downloads/BayesPlots/cumulative_lift_plot2.png", plot=cumulative_lift_plot2,  width=13, height=6)
...
ggsave(filename="/Users/colander1/Downloads/BayesPlots/cumulative_lift_priors_plot2.png", plot=cumulative_lift_priors_plot2,  width=13, height=6)
...
goalie_season <-  goalie_shot_level_prep %>%
    mutate(xG_ES = ifelse(SA_Goalie_gamestate == "EV",xG,0),
           GA_ES = ifelse(SA_Goalie_gamestate == "EV",as.numeric(goal)-1,0)) %>%
    group_by(SA_Goalie, season, Age) %>%
    summarise(NonRebound_Shots = sum(NonRebound_Shot),
              Shots = n(),
              GA = sum(as.numeric(goal)-1),
              ...
              xG_Lift_100Shots = (sum(xG) -sum(as.numeric(goal)-1)) / (n() / 100),
              xG_xR_Lift_100Shots = (sum(xG_FirstShot + xG_RB) - sum(as.numeric(goal)-1)) / (sum(NonRebound_Shot) / 100),
...
goalie_bayes_ranking <- function(shot_count_prior, young_age, old_inflator, seasons_prior, past_weight) {
...
test_season_loop <- function(szn) {
      ...
      goalie_target <- goalie_season %>%
              ...
              dplyr::select(SA_Goalie, Shots, NonRebound_Shots, posterior_lift, posterior_lift_xR, xG_Lift_100Shots, xG_xR_Lift_100Shots, SvPct, xG_ES_Lift_100Shots)
      ...
      goalie_prior_performance <- goalie_target %>%
              ...
              group_by(SA_Goalie) %>%
              mutate(init_weight = Age_Weight ** Season_Prior,
          ...
          summarise(weighted_posterior_lift = sum(weighted_posterior_lift),
                    weighted_mean_posterior_lift = weighted.mean(posterior_lift, weight),
                    weighted_posterior_lift_xR = sum(weighted_posterior_lift_xR),
                    weighted_mean_posterior_lift_xR = weighted.mean(posterior_lift_xR, weight),
                    ...
                    weighted_xG_ES_Lift_100Shots = weighted.mean(xG_ES_Lift_100Shots, weight * Shots),
...
library(boot)
bayes_corr <- corr(as.matrix(goalie_prior_performance[c("posterior_lift", "weighted_posterior_lift")]), w = (goalie_prior_performance$Shots))
bayes_w_corr <- corr(as.matrix(goalie_prior_performance[c("posterior_lift", "weighted_mean_posterior_lift")]), w = (goalie_prior_performance$Shots))

bayes_xR_corr <- corr(as.matrix(goalie_prior_performance[c("posterior_lift_xR", "weighted_posterior_lift_xR")]), w = (goalie_prior_performance$Shots))
bayes_xR_w_corr <- corr(as.matrix(goalie_prior_performance[c("posterior_lift_xR", "weighted_mean_posterior_lift_xR")]), w = (goalie_prior_performance$Shots))

svpct_corr <- corr(as.matrix(goalie_prior_performance[goalie_prior_performance$Shots > 550,c("SvPct", "weighted_SvPct")]), w = (goalie_prior_performance$Shots))
...
xGES_corr <- corr(as.matrix(goalie_prior_performance[c("xG_ES_Lift_100Shots", "weighted_xG_ES_Lift_100Shots")]), w = (goalie_prior_performance$NonRebound_Shots))
output <- data.frame(Shot_Count_Prior = shot_count_prior, 
                     ...
                     Bayes_xR_Corr = bayes_xR_corr,
                     Bayes_xR_weighted_Corr = bayes_xR_w_corr,
...
print(goalie_bayes_ranking(3000, 18, 2, 4, 0))


goalie_bayes_ranking(3000, 18, 2, 4, 1)

```

```{r}
grid <- expand.grid(shot_count_prior = seq(100, 4000, 300), young_age = seq(10,18,2), old_inflator = seq(0.5,3,0.5), seasons_prior = c(1,2,3,4,5), past_weight = c(0,1,2,4,6,8)) 

correlations_12600grid <- plyr::mdply(grid,goalie_bayes_ranking)

save(correlations_12600grid, file="~/Documents/CWA/Hockey Data/correlations_12600grid.RData")
...
correlations_sum <- correlations_12600grid %>%
        #dplyr::select(Bayes_Corr, Bayes_weighted_Corr, Bayes_xR_Corr, Bayes_xR_weighted_Corr, SvPct_Corr, xG_Corr, xGxR_Corr, xGES_Corr) %>%
        dplyr::select(ends_with("_Corr"),
                      -ends_with("weighted_Corr"),
                      -starts_with("xGES")) %>%
        rename(`Bayesian xG Lift Over Expected\n(Rebound Adjusted)` = Bayes_xR_Corr,
               `Bayesian xG Lift Over Expected` = Bayes_Corr,
               `xG Lift Over Expected\n(Rebound Adjusted)` = xGxR_Corr,
               `xG Lift Over Expected` = xG_Corr,
               `Save Percentage` = SvPct_Corr) %>%
        melt() %>%
        group_by(variable) %>%
        top_n(50) %>%
...
correlations_summary_plot <- correlations_sum %>%
        melt(id.var = "variable", variable.name = "var1") %>%
        group_by(var1) %>%
        mutate(var_rank = percent_rank(value)) %>%
        ggplot(aes(x=reorder(var1,-value), y=reorder(variable,value), label=paste0(round(value,3)))) +
...
ggsave(filename=paste0("/Users/colander1/Downloads/BayesPlots/correlations_summary_plot.png"), plot=correlations_summary_plot,  width=13, height=6)
...
season_projection <- function(shot_count_prior, young_age, old_inflator, seasons_prior, past_weight, season_project = "20172018") {
  ...
  goalie_projection <- goalie_shot_level_prep %>%
      ...
      mutate(season_projected = season_project,
             Age_Weight = as.numeric(1 - ((Age - young_age) * old_inflator) / 100),
             init_weight = Age_Weight ** Season_Prior,
             marcel_adjustor = (1 - (Season_Prior / (seasons_prior + past_weight) )),
             weight = init_weight * marcel_adjustor,
             weight = weight / sum(weight),
                 weighted_posterior_lift = weight * posterior_lift,
          ...
          summarise(weighted_posterior_lift = sum(weighted_posterior_lift),
                    ...
                    weighted_xG_xR_Lift_100Shots = weighted.mean(xG_xR_Lift_100Shots, weight * NonRebound_Shots),
                    weighted_SvPct = weighted.mean(SvPct, weight * Shots))
                    #actual_posterior_lift = max(posterior_lift),
...
sample <- goalie_projection %>% 
        filter(SA_Goalie == "CAREY PRICE") %>%
        select(SA_Goalie, season,Season_Prior, Age, Age_Weight, weight, posterior_lift, posterior_lift_xR, SvPct, weighted_posterior_lift, weighted_posterior_lift_xR) %>%
        rename(Season = season,
               Season_Weight = weight,
               Bayesian_Lift = posterior_lift,
               Bayesian_Lift_xR = posterior_lift_xR,
               Bayesian_Lift_Weighted = weighted_posterior_lift,
               Bayesian_Lift_xR_Weighted = weighted_posterior_lift_xR)
...
grid2 <- expand.grid(shot_count_prior = seq(100, 4000, 300), young_age = seq(10,18,2), old_inflator = seq(0.5,3,0.5), seasons_prior = c(1,2,3,4,5)) %>% head(10)

projections_2018_100 <- season_projection(shot_count_prior = 2000, young_age = 16, old_inflator = 2, seasons_prior = 4,  past_weight = 6) 

projections_2018_100 <- plyr::mdply(grid2,season_projection)
...
load("~/Documents/CWA/Hockey Data/correlations_2100grid.RData")
